...
void irq_handler_toggles(void* context, alt_u32 irqnum) {
    ...
    *de2_pio_redled18_base = 0x1;               //Sätt på första red-led
    ...
    save_value = alt_irq_interruptible(irqnum); //Tillåt interrupts med index samma som nuvarande och högre
    bigdelay();
    alt_irq_non_interruptible(save_value);      //Disabla interrupts igen
