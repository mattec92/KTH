...
main:
    ...
    #
	movia r8, keys4_base	#Flyttar keys4_base till r8
	movi r9, 0x1			#Flyttar 0x1 till r9
	stwio r9, 8(r8)			#Skriver en etta till interupt-masken
    #
	...
	ori r8, r8, 0x404		#Ettställer interrupt med index 10 och 2
...
exchand:
    nop
    nop
    rdctl	r10, estatus	#Läser in estatus
	PUSH	r10				#Pushar estatus
    push ea					#Pushar returadressen
    ...
    # (Condition 2).
noint:	push r8						#Pushar r8
		ldw r24, -4(r29)			#ladda in instruktionen innan exc skett
		movia r8, trapcode			#Ladda in OP-code för trap
		cmpeq r24, r8, r24			#Kolla om det är en trap som skett
		pop r8						#Popa r8
		bne r24, r0, exc_was_trap	#Hoppa om det är en trap
		br excend					#Annars returna
...
exc_was_interrupt:
    ...
    # This requirement is Nios2-specific.
   	pop ea					#Popa returadressen
    subi    ea,ea,4			#Dra bort 4 så att man hamnar på rätt instruktion
    push ea					#Pusha returadressen
    ...
    movia r9, keys4_base		#Ladda in keys4_base till r9
    ldw r8, 0(r9)				#Ladda in knapparnas värden till r8
    andi r8, r8, 0x1			#Maska fram första knappen
    rdctl r24, ipending			#Läs in ipending
    andi r24, r24, 0x4			#Maska fram biten för keys4
    bne r24, r0, keys4			#Om interrupten är för keys4, hoppa till keys4
...
keys4:	beq r8, r0, keys4_p		#Om knappen inte är intryckt, hoppa till 
		PUSHMOST				#Pusha registren
		movi r4, 'U'			#Flytta 'U' till r4
		call out_char_uart_0	#Skriv ut tecken
		POPMOST					#Popa registren
		br excend				#Hoppa ut
keys4_p:PUSHMOST				#osv
		movi r4, 'D'
...
excend:
	pop ea					#Popa returadressen från den ursprungliga/första exceptionen
	POP		r10				#Popa ursprungliga estatus, re-enabla traps
	wrctl	estatus, r10	#Skriv estatus
    eret
#
########################################
